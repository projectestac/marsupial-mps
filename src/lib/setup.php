...
if (empty($CFG->usesid) || !empty($_COOKIE['MoodleSession' . $CFG->sessioncookie])) {
    if ($SESSION != NULL) {
        if (empty($_COOKIE['MoodleSessionTest' . $CFG->sessioncookie])) {
            ...
            if (isset($SESSION->session_test) && $_COOKIE['MoodleSessionTest' . $CFG->sessioncookie] != $SESSION->session_test) {
                report_session_error();
            }
        }
    }
...
if ($SESSION !== NULL && isset($_GET['lang']) && ($lang = clean_param($_GET['lang'], PARAM_SAFEDIR))) {
    if (file_exists($CFG->dataroot . '/lang/' . $lang) or file_exists($CFG->dirroot . '/lang/' . $lang)) {
        ...
        if (file_exists($CFG->dataroot . '/lang/' . $lang . '_utf8') or
    ...
    if (empty($USER->id)) {  // Ignore anyone logged in
        if (!empty($_SERVER['HTTP_USER_AGENT'])) {
            if (strpos($_SERVER['HTTP_USER_AGENT'], 'Googlebot') !== false) {
                ...
                if (strpos($_SERVER['HTTP_USER_AGENT'], 'google.com') !== false) { // Google
                    $USER = guest_user();
                } else {
                    if (strpos($_SERVER['HTTP_USER_AGENT'], 'Yahoo! Slurp') !== false) {  // Yahoo
                        $USER = guest_user();
                    } else {
                        if (strpos($_SERVER['HTTP_USER_AGENT'], '[ZSEBOT]') !== false) {  // Zoomspider
                            $USER = guest_user();
                        } else {
                            if (strpos($_SERVER['HTTP_USER_AGENT'], 'MSNBOT') !== false) {  // MSN Search
                                $USER = guest_user();
                            }
                        }
                    }
